...
export function App() {
  ...
  const account = dashboard.data.accounts[0]
  if (!account) {
    return (
      <main className="app-shell">
  ...
  const latestScan = dashboard.data.scans[0]
      ...
      <div className="dashboard">
        ...
        <section className="summary-grid" aria-label="账号摘要">
          ...
          <a
            ...
            href="/action-items"
            aria-label={`互关后取消关注 ${dashboard.data.actionItems.length} 项`}
          >
            <span className="card-label">重点待处理</span>
            <strong>互关后取消关注 {dashboard.data.actionItems.length} 项</strong>
            <span>查看并确认事件 →</span>
          </a>
        </section>
        {latestScan?.status === 'FAILED' && (
